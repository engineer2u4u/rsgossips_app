...
export default function InfluencerServicesList() {
  ...
  const filtered = useMemo(() => {
    ...
    return services.filter(s => {
      if (selectedTags.length && s.tag && !selectedTags.includes(s.tag))
        return false;
              ...
              return (
                <TouchableOpacity
                  ...
                  className={`px-3 py-1.5 rounded-full ${
                    active ? 'bg-[#E60076]' : 'bg-white border border-slate-200'
                  }`}
                >
                  <Text
...
function ServiceCard({
  ...
  // Shadow lives on an outer wrapper so the inner card can keep
  // overflow-hidden (needed to clip the hero image to the rounded corners)
  // without also clipping the drop shadow into invisibility.
  return (
    <View
      style={{
        borderRadius: 16,

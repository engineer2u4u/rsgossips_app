...
export default function BrandMatchPrompt() {
  const { user } = useAuth();
  // The navy card now stretches with width: '100%' inside the parent's own
  // padding — no explicit screen-width pin needed (BVLinearGradient sizes
  // fine here because the width is declared, not derived from content).
  ...
  const run = async () => {
    ...
    try {
      const data = await invokeFn<{
        influencers?: MatchRow[];
        summary?: string;
        error?: string;
      }>('landing-match', { prompt: text });
    ...
    <View style={{ paddingHorizontal: 0 }}>
      ...
      >
        ...
        >
          ...
          >
            One line — niche, city, budget. We rank verified creators in
            minutes.
          </Text>
      ...
      {results.length > 0 ? (
        <View
          style={{ marginTop: 12, gap: 8, marginLeft: 20, marginRight: 20 }}
        >
          {summary ? (
            <Text
              style={{
          ...
          {shortlistCount > 0 ? (
            ...
            <View
              ...
              >
                ...
                <Text
                  style={{ color: '#6A66C9', fontSize: 12, fontWeight: '800' }}
                >
                  Invite to campaign
                </Text>
              </Pressable>
      ...
      <CampaignPickerModal
        ...
        onDone={result => {
          ...
          setToast(
            `Invited ${invited} creator${invited === 1 ? '' : 's'} to your campaign.`,
          );
          setShortlist({});

...
- **Payment methods CRUD** — always route add/edit through the `register-payout-method` edge function (not a direct `supabase.from('payment_methods').insert(...)`). The edge function creates the RazorpayX Contact + Fund Account, runs name-match validation, and inserts the row with every NOT NULL column populated.

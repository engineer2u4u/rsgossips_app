...
- **iOS layout pass — do this on every UI change, unprompted.** iOS is where spacing and button sizes come out wrong, and the same handful of causes keep recurring. After touching any UI, run `npm run check:ios` (`scripts/check-ios-layout.js`) and work through this list; do not wait to be asked for "an iOS pass".
  1. **Never let a `LinearGradient` size itself.** `react-native-linear-gradient` 2.8.3 has no Fabric support, so on RN 0.84's New Architecture a gradient that derives its size from its own `padding` + children **collapses on iOS** and clips the label ("Match" → "Mat"). The wrapper (`View`/`Pressable`/`TouchableOpacity`) owns `padding`/`height`/`borderRadius` + `overflow: 'hidden'`; the gradient becomes an absolute-fill background sibling, with the label/icon after it. Safe as-is: explicit `width` **and** `height` (icon chips, avatars, FABs), a full-bleed card given an explicit `width` via `useWindowDimensions`, and `%`-width progress fills. See `c53ceb3`.
  2. **Bottom insets come from `useSafeAreaInsets()`**, never a hard-coded `Platform.OS === 'ios' ? 34 : 20` — see the Modals bullet below.
  3. **`SafeAreaView` reports zero insets inside a `<Modal>` on iOS** (the modal mounts in its own native window, outside the provider tree). Use the `useSafeAreaInsets()` hook, which reads React context and works there. A full-screen modal needs `paddingTop: Platform.OS === 'ios' ? insets.top : 0` or its header sits under the notch.
  4. **No emoji in UI text** — they render as `?` tofu on the device font stack. Use `lucide-react-native` icons.
  5. **Buttons get an explicit `height`** rather than relying on `py-*` alone; iOS text metrics differ, so padding-only buttons come out shorter than the Android reference.
  6. **`KeyboardAvoidingView`** uses `behavior="padding"` on iOS (`'height'`/undefined on Android) or forms sit under the keyboard.
- **Modals / bottom sheets** — `targetSdkVersion 36` forces edge-to-edge on Android 15+, so a `<Modal>` draws under the system nav bar (and under the home indicator on iOS). Pad the sheet's footer/bottom with `useSafeAreaInsets().bottom` (e.g. `paddingBottom: 16 + insets.bottom`); never hard-code `Platform.OS === 'ios' ? 34 : 20`. The hook works inside a Modal because it reads React context; the native `SafeAreaView` reports zero insets there on iOS. A `ScrollView` inside a `maxHeight`-capped sheet needs `style={{flexShrink: 1}}`: Yoga's default `flexShrink: 0` lets it overflow the cap, so it never scrolls and the footer gets pushed off-screen. A ScrollView nested in another needs `nestedScrollEnabled` on Android.
